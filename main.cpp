...
// DECLARANDO CONJUNTOS
    ...
    for(i = 0; i < I; i++)
    {
        ...
        stringstream var;
        var << "x["<<i+1<<"]: ";
        x[i].setName(var.str().c_str()); //str() - converte para string e c_str() converte para char*
    ...
    for(i = 0; i < I; i++)
    {
        fo += p[i]*x[i]; //Somatorio dos valores, por isso +=
    }
    
    //IloMinimize e IloMaximize
    modelo.add(IloMinimize(env, fo)); //Minimize o FO no ambiente env
